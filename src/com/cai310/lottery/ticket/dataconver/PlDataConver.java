...
public class PlDataConver {
	...
	public static List<TicketDTO> converData(PrintInterfaceDTO printData, Date createTime) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		...
		if(printData.getBetType()==PlPlayType.GroupSum.ordinal()){//组选和值需要区分组选3与组选6				
			List<String> groupSums = null;			
			for(String content:contents){
				plCompoundContent = JsonUtil.getObject4JsonString(content, PlCompoundContent.class);
				groupSums = plCompoundContent.getGroupSumList();					
			...
			//保存组三
			if(!group3s.isEmpty()){
				ticketNew = createTicketDTONew(printData,createTime,group3s,PlPlayType.Group3);
				resultList.add(ticketNew);
			}			
			//保存组六
			if(!group6s.isEmpty()){
				ticketNew = createTicketDTONew(printData,createTime,group6s,PlPlayType.Group6);
				resultList.add(ticketNew);
			}
		}else if(printData.getBetType()==PlPlayType.P3DirectKuadu.ordinal()){//直选跨度改为单式直选
			...
			for(String content:contents){
				plCompoundContent = JsonUtil.getObject4JsonString(content, PlCompoundContent.class);
				directKuadus = plCompoundContent.getDirectKuaduList();
			...
			ticketNew = createTicketDTONew(printData,createTime,contentList,PlPlayType.P3Direct);
			resultList.add(ticketNew);			
		}else if(printData.getBetType()==PlPlayType.P3Group3Kuadu.ordinal()){//组三跨度改为单式组三
			...
			for(String content:contents){
				plCompoundContent = JsonUtil.getObject4JsonString(content, PlCompoundContent.class);
				g3Kuadus = plCompoundContent.getG3KuaduList();
			...
			ticketNew = createTicketDTONew(printData,createTime,contentList,PlPlayType.Group3);
			resultList.add(ticketNew);	
		}else if(printData.getBetType()==PlPlayType.P3Group6Kuadu.ordinal()){//组六跨度改为单式组六
			...
			for(String content:contents){
				plCompoundContent = JsonUtil.getObject4JsonString(content, PlCompoundContent.class);
				g6Kuadus = plCompoundContent.getG6KuaduList();
			...
			contentList.addAll(resultSet);
			ticketNew = createTicketDTONew(printData,createTime,contentList,PlPlayType.Group6);
			resultList.add(ticketNew);	
		}
		return resultList;
	}
	 ...
	 * @throws NoSuchMethodException
	 */
	private static TicketDTO createTicketDTONew(PrintInterfaceDTO printData, Date createTime, List<String> contentList,PlPlayType playType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
