...
public class LczcRlygUtil extends RlygUtil {
	...
	public LczcRlygUtil(Ticket ticket) {
		super(ticket);
	}
	public LczcRlygUtil(){
		super();
	...
	public Map<String,String> getLotCode(Ticket ticket) throws DataException{
		...
		if(SalesMode.SINGLE.equals(ticket.getMode())){
			...
			for (String line : ticketArr) {
				if (line.length() != (ZcUtils.getMatchCount(ticket.getLotteryType())*2))
			...
			if (betArr.length != (ZcUtils.getMatchCount(ticket.getLotteryType())*2))
				throw new DataException("单式选项不符");
