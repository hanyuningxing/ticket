...
public class DanTuoDataConver
{
  ...
  public DanTuoDataConver(int combinNum, List<String> danCorrectList, List<String> unDanCorrectList)
  {
  ...
  class SplitExtensionCombCallBack implements ExtensionCombCallBack {
    ...
    public boolean run(boolean[] comb1, int m1, boolean[] comb2, int m2) {
      ...
      for (int i = 0; i < comb1.length; ++i) {
        if (comb1[i]) {
          combList.add((String)DanTuoDataConver.this.danCorrectList.get(i));
          if (combList.size() == m1)
            break;
        }
      }
      ...
      for (int i = 0; i < comb2.length; ++i) {
        if (comb2[i]) {
          combList.add((String)DanTuoDataConver.this.unDanCorrectList.get(i));
          if (combList.size() == n) {
            break;
          }
        }
      ...
      DanTuoDataConver.this.resultList.add(combList);
      return false;
    }
  }
}