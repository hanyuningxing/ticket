...
public class ZunaoDczcSpVisitor extends VisitorSupport{
	...
	private ZunaoDczcSp zunaoDczcSp;
	private List<ZunaoDczcSp> zunaoDczcSpList;
	public void visit(Attribute node){
		if("matchId".equals(node.getName())){
			if(StringUtils.isNotBlank(node.getText())){
				zunaoDczcSp.setMatchId(node.getText().trim());
			}
		}
		if("matchtime".equals(node.getName())){
			if(StringUtils.isNotBlank(node.getText())){
				zunaoDczcSp.setMatchtime(node.getText().trim());
			}
		}
		if("sp".equals(node.getName())){
			if(StringUtils.isNotBlank(node.getText())){
				zunaoDczcSp.setSp(node.getText().trim());
			}
		}
		if(null!=zunaoDczcSp&&StringUtils.isNotBlank(zunaoDczcSp.getSp())){
			zunaoDczcSpList.add(zunaoDczcSp);
		}
	}
	public void visit(Element node){
		if("spInfo".equals(node.getName())){
			if(null==zunaoDczcSpList)zunaoDczcSpList = Lists.newArrayList();
			zunaoDczcSp = new ZunaoDczcSp();
		}
	}
	public ZunaoDczcSp getZunaoDczcSp() {
		return zunaoDczcSp;
	}
	public void setZunaoDczcSp(ZunaoDczcSp zunaoDczcSp) {
		this.zunaoDczcSp = zunaoDczcSp;
	}
	public List<ZunaoDczcSp> getZunaoDczcSpList() {
		return zunaoDczcSpList;
	}
	public void setZunaoDczcSpList(List<ZunaoDczcSp> zunaoDczcSpList) {
		this.zunaoDczcSpList = zunaoDczcSpList;
	}
