...
public enum PassType {
	...
	private PassType(int matchCount, int[] passMatchs,String cpText) {
		...
		this.passMatchList = Lists.newArrayList();
		for (Integer pass : this.passMatchs) {
			this.passMatchList.add(pass);
		}
	...
	public static List<PassType> findPassTypes(List<Integer> passList, int maxMatchSize) {
		List<PassType> list = new ArrayList<PassType>();
		for (PassType passType : PassType.values()) {
			if (passType.getMatchCount() <= maxMatchSize && passType.getMatchCount() > 1) {
				if (passList.containsAll(passType.passMatchList)) {
					list.add(passType);
				}
			}
		}
	 ...
	 * @return 过关方式类型
	 */
	public static PassType getPassType(long value) {
		for (PassType spPassType : values()) {
