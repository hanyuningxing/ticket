...
public class PrizeForecastItem implements Serializable {
	...
	private final List<JczqExtraMatchItem> danlist;
	private final List<JczqExtraMatchItem> undanlist;

	public PrizeForecastItem(Integer multiple, final List<JczqExtraMatchItem> list, Integer danMinHit,
			Integer danMaxHit, final List<Integer> passMatchList, final boolean min) {
		super();
		this.matchSize = list.size();

		this.danlist = new ArrayList<JczqExtraMatchItem>();
		this.undanlist = new ArrayList<JczqExtraMatchItem>();
		for (JczqExtraMatchItem item : list) {
			if (item.isDan())
	...
	private class ExtendCombCallBack implements ExtensionCombCallBack {
		private List<List<JczqExtraMatchItem>> detailList = new ArrayList<List<JczqExtraMatchItem>>();
		...
		public boolean run(boolean[] comb1, int m1, boolean[] comb2, int m2) {
			int n = m1 + m2;
			List<JczqExtraMatchItem> combList = new ArrayList<JczqExtraMatchItem>();
			double linePrize = 1;
			StringBuilder line = new StringBuilder();
			String passText = JczqConstant.PASS_TEXT[n - 1];
			line.append('[').append(passText).append(']');
			for (int i = 0; i < comb1.length; i++) {
				if (comb1[i]) {
					JczqExtraMatchItem item = danlist.get(i);
					combList.add(item);

					Double sp;
					if (item.isCancel()) {
						sp = item.selectedCount() * JczqConstant.CANCEL_MATCH_RESULT_SP;
						StringBuilder cancelSpText = new StringBuilder();
						for (int j = 0; j < item.selectedCount(); j++) {
							cancelSpText.append(CANCEL_SP_FORMAT.format(JczqConstant.CANCEL_MATCH_RESULT_SP)).append('+');
						}
			...
			for (int i = 0; i < comb2.length; i++) {
				if (comb2[i]) {
					JczqExtraMatchItem item = undanlist.get(i);
					combList.add(item);
			...
			if (n == 1 && combList.get(0).isCancel()) {
				...
				if(n==1){//单关
					linePrize *= multiple;
					line.append(multiple).append(SIGN_MULTIPLE).append('=').append(
							MONEY_FORMAT.format(linePrize));
				} else {
				linePrize *= multiple * UNITS_MONEY ;
				line.append(multiple).append(SIGN_MULTIPLE).append(UNITS_MONEY).append('=').append(
						MONEY_FORMAT.format(linePrize));
				}
			}
