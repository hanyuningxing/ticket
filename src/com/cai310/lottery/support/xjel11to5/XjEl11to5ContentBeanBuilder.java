 ...
 * @version 1.0
 */
public class XjEl11to5ContentBeanBuilder {
	/**
	 * ContentBean复式构建器
	 * 
	 * @param schemeContent
	 ...
	 */
	public static ContentBean buildCompoundContentBean(String schemeContent, XjEl11to5PlayType xjel11to5PlayType)
			throws DataException {
		// /复式方案内容注数：
		...
		int allUnits = 0;
		List<XjEl11to5CompoundContent> all = new ArrayList<XjEl11to5CompoundContent>(10);
		...
		for (String str : arr) {
			...
			XjEl11to5CompoundContent lineContent = new XjEl11to5CompoundContent();
			if (xjel11to5PlayType.equals(XjEl11to5PlayType.NormalOne)) {
				patt = Pattern.compile(XjEl11to5Constant.GENERAL_COMPOUND_REGEX);
				...
				betList = Arrays.asList(betStr.split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
				...
				countUnits = xjel11to5PlayType.countUnit(betList);
				if (units != countUnits)
				...
				patt = Pattern.compile(XjEl11to5Constant.DIRECT2_COMPOUND_REGEX);
				matcher = patt.matcher(str);
				if (!matcher.matches())
					throw new DataException("方案内容格式不正确.");
				String unitsStr = matcher.group(1);
				int units = Integer.parseInt(unitsStr);
				String betStr = matcher.group(2).trim();
				String[] betArr = betStr.split(XjEl11to5Constant.SEPARATOR_FOR_);
				bet1List = Arrays.asList(betArr[0].split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
				bet2List = Arrays.asList(betArr[1].split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
				...
				countUnits = xjel11to5PlayType.countUnit(bet1List, bet2List);
				if (units != countUnits)
					throw new DataException("注数与系统计算的不一致.");
				...
				patt = Pattern.compile(XjEl11to5Constant.DIRECT3_COMPOUND_REGEX);
				matcher = patt.matcher(str);
				if (!matcher.matches())
					throw new DataException("方案内容格式不正确.");
				String unitsStr = matcher.group(1);
				...
				bet1List = Arrays.asList(betArr[0].split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
				bet2List = Arrays.asList(betArr[1].split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
				bet3List = Arrays.asList(betArr[2].split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
				for (String num : bet1List) {
					num = num.trim();
					int number = Integer.parseInt(num);
					if (number < 1 || number > 11)
				...
				countUnits = xjel11to5PlayType.countUnit(bet1List, bet2List, bet3List);
				if (units != countUnits)
					throw new DataException("注数与系统计算的不一致.");
				lineContent.setBet1List(bet1List);
				lineContent.setBet2List(bet2List);
				...
				lineContent.setUnits(units);
					...
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.ForeTwoGroup)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.RandomThree)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.ForeThreeGroup)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.RandomFour)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.RandomFive)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.RandomSix)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.RandomSeven)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.RandomEight)) {
				boolean hasDan = false;// 是否为含胆码格式
				if (str.indexOf(XjEl11to5Constant.SEPARATOR_DAN_FOR_NUMBER) >= 0) {
					patt = Pattern.compile(XjEl11to5Constant.GENERAL_COMPOUND_DAN_REGEX);
					hasDan = true;
				} else {
					patt = Pattern.compile(XjEl11to5Constant.GENERAL_COMPOUND_REGEX);
				}
				...
				String betStr = matcher.group(2).trim();
				if (hasDan) {
					String[] tempArr = betStr.split(XjEl11to5Constant.SEPARATOR_DAN_FOR_NUMBER);
					betList = Arrays.asList(tempArr[0].split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
					for (String num : betList) {
						num = num.trim();
						int number = Integer.parseInt(num);
						if (number < 1 || number > 11)
					...
					betList = Arrays.asList(tempArr[1].split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
					for (String num : betList) {
						num = num.trim();
						int number = Integer.parseInt(num);
						if (number < 1 || number > 11)
					...
					lineContent.setBetList(betList);
					countUnits = xjel11to5PlayType.countDanUnit(lineContent.getBetDanList(), lineContent.getBetList());
				} else {
					betList = Arrays.asList(betStr.split(XjEl11to5Constant.SEPARATOR_FOR_NUMBER));
					for (String num : betList) {
						num = num.trim();
	 ...
	 * @throws DataException
	 */
	public static ContentBean buildSingleContentBean(String schemeContent, XjEl11to5PlayType xjel11to5PlayType)
			throws DataException {
		if (StringUtils.isBlank(schemeContent))
			throw new DataException("方案内容为空.");
		...
		for (String str : arr) {
			...
			if (xjel11to5PlayType.equals(XjEl11to5PlayType.NormalOne)) {
				...
				throw new DataException("任选1不支持单式投注.");
					...
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.RandomTwo)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.ForeTwoGroup)) {
				// 两个号码
				single_regex = XjEl11to5Constant.CHOOSE2_SINGLE_REGEX;
			} else if (xjel11to5PlayType.equals(XjEl11to5PlayType.RandomThree)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.ForeThreeGroup)
					|| xjel11to5PlayType.equals(XjEl11to5PlayType.ForeThreeDirect)) {
				// 三个号码
				single_regex = XjEl11to5Constant.CHOOSE3_SINGLE_REGEX;
			} else if (xjel11to5PlayType.equals(XjEl11to5PlayType.RandomFour)) {
				// 四个号码
				single_regex = XjEl11to5Constant.CHOOSE4_SINGLE_REGEX;
			} else if (xjel11to5PlayType.equals(XjEl11to5PlayType.RandomFive)) {
				// 五个号码
				single_regex = XjEl11to5Constant.CHOOSE5_SINGLE_REGEX;
			} else if (xjel11to5PlayType.equals(XjEl11to5PlayType.RandomSix)) {
				// 五个号码
				single_regex = XjEl11to5Constant.CHOOSE6_SINGLE_REGEX;
			} else if (xjel11to5PlayType.equals(XjEl11to5PlayType.RandomSeven)) {
				// 五个号码
				single_regex = XjEl11to5Constant.CHOOSE7_SINGLE_REGEX;
			} else if (xjel11to5PlayType.equals(XjEl11to5PlayType.RandomEight)) {
				// 五个号码
				single_regex = XjEl11to5Constant.CHOOSE8_SINGLE_REGEX;
			} else {
				throw new DataException("玩法不正确.");
			}
