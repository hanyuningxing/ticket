...
public enum GdEl11to5PlayType {
	...
	NormalOne("选一", Integer.valueOf("13"), 1) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
				int multiple) {
			GdEl11to5WinUnits gdEl11to5WinUnits = new GdEl11to5WinUnits();
			int resultNum1 = results[0];
			List<Integer> bets1 = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBetList());
			if (bets1.contains(resultNum1)) {
				gdEl11to5WinUnits.addNormalOneWinUnits(multiple);
			}
	...
	RandomTwo("选二", Integer.valueOf("6"), 2) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			int danSelected = 0;
			int selected = 0;
			if (gdEl11to5CompoundContent.getBetDanList() != null && !gdEl11to5CompoundContent.getBetDanList().isEmpty()) {
				List<Integer> bets = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBetDanList());
			...
			List<Integer> bets = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBetList());
			for (Integer result : results) {
				if (bets.contains(result)) {
					hits++;
				}
			...
			gdEl11to5WinUnits.addRandomTwoWinUnits(calHit(2, 2, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	ForeTwoGroup("前二组选", Integer.valueOf("65"), 2) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			selected = bets.size();
			gdEl11to5WinUnits.addForeTwoGroupWinUnits(calHit(2, 2, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	ForeTwoDirect("前二直选", Integer.valueOf("130"), 1) {
		...
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			int resultNum1 = results[0];
			int resultNum2 = results[1];
			List<Integer> bets1 = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBet1List());
			List<Integer> bets2 = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBet2List());
			if (bets1.contains(resultNum1) && bets2.contains(resultNum2)) {
				gdEl11to5WinUnits.addForeTwoDirectWinUnits(multiple);
			}
			return gdEl11to5WinUnits;
		}
	...
	RandomThree("任选三 ", Integer.valueOf("19"), 3) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			gdEl11to5WinUnits.addRandomThreeWinUnits(calHit(3, 3, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	ForeThreeGroup("选三前组", Integer.valueOf("195"), 3) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			selected = bets.size();
			gdEl11to5WinUnits.addForeThreeGroupWinUnits(calHit(3, 3, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	ForeThreeDirect("选三前直", Integer.valueOf("1170"), 1) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			List<Integer> bets1 = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBet1List());
			List<Integer> bets2 = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBet2List());
			List<Integer> bets3 = NumUtils.toIntegerList(gdEl11to5CompoundContent.getBet3List());
			if (bets1.contains(resultNum1) && bets2.contains(resultNum2) && bets3.contains(resultNum3)) {
				gdEl11to5WinUnits.addForeThreeDirectWinUnits(multiple);
			}
			return gdEl11to5WinUnits;
		}
	...
	RandomFour("任选四 ", Integer.valueOf("78"), 4) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			gdEl11to5WinUnits.addRandomFourWinUnits(calHit(4, 4, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	RandomFive("任选五", Integer.valueOf("540"), 5) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			gdEl11to5WinUnits.addRandomFiveWinUnits(calHit(5, 5, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	RandomSix("任选六", Integer.valueOf("90"), 6) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			gdEl11to5WinUnits.addRandomSixWinUnits(calHit(6, 5, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	RandomSeven("任选七", Integer.valueOf("26"), 7) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			gdEl11to5WinUnits.addRandomSevenWinUnits(calHit(7, 5, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	RandomEight("任选八", Integer.valueOf("9"), 8) {
		public GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			...
			gdEl11to5WinUnits.addRandomEightWinUnits(calHit(8, 5, danSelected, danHits, selected, hits) * multiple);
			return gdEl11to5WinUnits;
		}
	...
	private GdEl11to5PlayType(String typeName, Integer prize, Integer lineLimit) {
	...
	public abstract GdEl11to5WinUnits calcWinUnit(Integer[] results, GdEl11to5CompoundContent gdEl11to5CompoundContent,
			int multiple);
	...
	public static GdEl11to5PlayType getSdEl11to5PlayType(String typeName) {
		for (GdEl11to5PlayType type : GdEl11to5PlayType.values()) {
